 ...
 * @param event Concierto opcional preseleccionado para la reserva.
 */
  ...
  const eventBadge = event
    ? `
      <div class="mb-4 p-3 bg-zinc-900/90 border border-zinc-800 rounded-lg flex items-center justify-between gap-3">
        ...
        <span class="text-xs font-semibold text-zinc-400 shrink-0">${event.artist}</span>
      </div>
    `
    : '';
    ...
    <section class="w-full bg-zinc-950 border border-zinc-800/90 rounded-xl p-5 md:p-6 shadow-xl relative overflow-hidden">
      ...
      <header class="mb-4">
        ...
        <p class="text-xs text-zinc-400">
          Asegura tu lugar en las mejores tocatas Jazz y RnB independientes.
        </p>
      </header>
...
function renderSuccessState(
  ...
  const eventName = event ? event.title : 'Evento NeonPulse';
  ...
  sectionElement.innerHTML = `
    <div class="text-center py-6 px-4 flex flex-col items-center gap-2">
      ...
      <p class="text-xs text-zinc-300 max-w-md">
        ...
        <strong class="text-red-500 font-bold">${eventName}</strong>.
      </p>
...
export function createBookingFormElement(
  ...
  if (form && emailInput && cantidadInput && errorBlock) {
    form.addEventListener('submit', (e) => {
      ...
      renderSuccessState(sectionElement, email, cantidad, event);
      if (onSubmitSuccess) {
        onSubmitSuccess({ email, cantidad, event });
      }
    });
  }
